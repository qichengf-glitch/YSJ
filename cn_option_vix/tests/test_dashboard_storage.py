...
def test_halfday_explicit_start_and_moving_averages(tmp_path: Path):
    ...
    expected_20 = pd.Series([21.0 + i for i in range(45, 65)])
    expected_60 = pd.Series([21.0 + i for i in range(5, 65)])
    assert overall["avg_20"] == expected_20.mean()
    assert overall["variance_20"] == expected_20.var(ddof=1)
    assert overall["avg_60"] == expected_60.mean()
    assert overall["variance_60"] == expected_60.var(ddof=1)


def test_moving_average_snapshot_computes_relative_spread_statistics(tmp_path: Path):
    from cn_option_vix.web.storage import moving_average_snapshot

    db = tmp_path / "live.sqlite"
    overall_values = [20.0, 21.0, 22.0, 23.0]
    hard_values = [21.0, 23.0, 25.0, 27.0]
    rows = [
        _row(f"{day.date()} 15:00", overall=overall, hard=hard)
    ...
    upsert_points(rows, resolution="halfday", source="test", db_path=db)
    upsert_points(
        [_row("2026-07-08 13:20", overall=30.0, hard=35.5)],
        resolution="5m",
        source="test",
        db_path=db,
    )
