...
async function proxyToVixBackend(request: NextRequest, context: RouteContext) {
  ...
  try {
    ...
    const message = error instanceof Error ? error.message : "CN VIX backend unavailable";
    return NextResponse.json(
      {
        detail: "CN VIX backend is not reachable. Start cn_option_vix on port 8765 or set CN_VIX_BACKEND_URL.",
        error: message,
      },
