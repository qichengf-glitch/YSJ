...
function pmMeta(m, mode='up') {
  ...
  if (mode === 'volume') {
    ...
    const volPct = ratio ? (ratio - 1) * 100 : null;
    const avg = m.volume_10d_avg ? fmtMoney(m.volume_10d_avg) : '—';
    const pctText = volPct === null ? '—' : `${volPct >= 0 ? '+' : ''}${volPct.toFixed(0)}%`;
    return `${escapeHtml(m.bucket_label)} · 24h ${fmtMoney(m.volume_24h || 0)} / avg ${avg} · 波动 ${pctText}`;
  }
...
function pmTopLists(items) {
  const copy = [...items];
  let volume = copy.filter(m => (m.volume_spike_ratio || 0) >= 1.4)
    .sort((a,b) => (b.volume_spike_ratio || 0) - (a.volume_spike_ratio || 0) || (b.volume_24h || 0) - (a.volume_24h || 0));
  if (!volume.length) volume = copy.filter(m => (m.volume_24h || 0) > 0).sort((a,b) => (b.volume_24h || 0) - (a.volume_24h || 0));
  return {
    up: copy.filter(m => (m.change_7d_pp || 0) > 0).sort((a,b) => (b.change_7d_pp || 0) - (a.change_7d_pp || 0) || (b.volume_7d || 0) - (a.volume_7d || 0)).slice(0, 2),
    down: copy.filter(m => (m.change_7d_pp || 0) < 0).sort((a,b) => (a.change_7d_pp || 0) - (b.change_7d_pp || 0) || (b.volume_7d || 0) - (a.volume_7d || 0)).slice(0, 2),
    volume: volume.slice(0, 6),
  };
}
async function setPmBucket(bucket) {
  ...
  try { state.whaleDaily = await api(`/api/prediction-markets/whales/daily?bucket=${encodeURIComponent(state.pmBucket)}`); } catch (_) {}
  renderPrediction();
}

function renderDailyEvents(items) {
  if (!items || !items.length) return `<div class="empty">暂无近24小时交易数据</div>`;
  return items.slice(0, 6).map((m, idx) => `
    <div class="daily-row" onclick='openPmDrawer(${JSON.stringify(m.condition_id)})'>
      <span class="rank">${idx + 1}</span>
      <div class="daily-main"><div class="compact-title">${escapeHtml(m.question || m.event_title || '')}</div></div>
      <b>${fmtMoney(m.volume_24h || 0)}</b>
    </div>`).join('');
}
...
function fillWhaleSparkRows(points) {
  const raw = (points || []).filter(p => p && (p.yes !== undefined || p.no !== undefined))
    .map(p => ({
      time: p.time || p.date || '',
      date: String(p.date || p.time || '').slice(0, 10),
      yes: Number(p.yes || 0),
      no: Number(p.no || 0),
  ...
  for (let i = 9; i >= 0; i--) {
    ...
    rows.push({time: key, date: key, yes: 0, no: 0, total: 0, observed: false});
  }
  return rows;
}
function renderWhaleSparkline(points) {
  const rows = fillWhaleSparkRows(points);
  const width = 180, height = 54, pad = 5;
  const vals = rows.flatMap(p => [Number(p.yes || 0), Number(p.no || 0)]);
  const max = Math.max(1, ...vals);
  const x = (i) => pad + (i / Math.max(1, rows.length - 1)) * (width - pad * 2);
  const y = (v) => height - pad - (Number(v || 0) / max) * (height - pad * 2);
  const pts = (field) => rows.map((r, i) => `${x(i).toFixed(1)},${y(r[field]).toFixed(1)}`).join(' ');
  return `<svg class="whale-spark" viewBox="0 0 ${width} ${height}">
    <line x1="${pad}" y1="${height-pad}" x2="${width-pad}" y2="${height-pad}" class="spark-axis" />
    <polyline points="${pts('yes')}" class="spark-yes" />
    <polyline points="${pts('no')}" class="spark-no" />
    <text x="6" y="12" class="spark-label yes">YES</text>
    <text x="6" y="26" class="spark-label no">NO</text>
  </svg>`;
}
...
function renderPrediction() {
  ...
  const daily = state.whaleDaily || {events_traded_24h: [], top_traders_24h: []};
  if ($('pmDailyEvents')) $('pmDailyEvents').innerHTML = renderDailyEvents(daily.events_traded_24h || []);
  if ($('pmDailyTraders')) $('pmDailyTraders').innerHTML = renderDailyTraders(daily.top_traders_24h || []);
  $('pmTopUp').innerHTML = renderPmCompact(tops.up, 'up');
  $('pmTopDown').innerHTML = renderPmCompact(tops.down, 'down');
  $('pmVolumeLeaders').innerHTML = renderVolumeRadar(tops.volume);
  renderPmTable();
}
...
function renderVolumeRadar(items) {
  if (!items.length) return `<div class="empty">暂无成交异动。同步 Polymarket 后按 24h volume / rolling average 展示波动率。</div>`;
  return items.slice(0, 7).map((m, idx) => {
    const ratio = Number(m.volume_spike_ratio || 0);
    const avg = Number(m.volume_10d_avg || 0);
    const volPct = ratio ? (ratio - 1) * 100 : null;
    const absPct = volPct === null ? 0 : Math.abs(volPct);
    const barPct = Math.min(100, Math.max(8, absPct / 250 * 100));
    ...
    return `<div class="volume-radar-item" onclick='openPmDrawer(${JSON.stringify(m.condition_id)})'>
      ...
      <div class="volume-radar-main">
        <div class="compact-title">${pmQuestion(m, 140)}</div>
        <div class="compact-sub">${escapeHtml(m.bucket_label)} · 24h ${fmtMoney(m.volume_24h || 0)} / avg ${avg ? fmtMoney(avg) : '—'} · 7D ${fmtPP(m.change_7d_pp)}</div>
        <div class="volume-bar ${hot ? 'hot' : 'quiet'}"><span style="width:${barPct}%"></span></div>
      </div>
      <div class="volume-ratio ${hot ? 'hot' : quiet ? 'quiet' : ''}"><span>波动率</span><b>${pctText}</b><em>${escapeHtml(src)}</em></div>
    </div>`;
  }).join('');
}
function renderPmTable() {
  ...
  $('pmMarketsTable').innerHTML = `
    <table class="pm-table">
      <thead><tr><th>主线</th><th>事件 / Option</th><th>Prob</th><th>7D</th><th>1D</th><th>7D Vol</th><th>24h Vol</th><th>波动率</th></tr></thead>
      <tbody>${items.map(m => {
        const ratioVal = Number(m.volume_spike_ratio || 0);
        const ratio = ratioVal ? `${((ratioVal - 1) * 100 >= 0 ? '+' : '')}${((ratioVal - 1) * 100).toFixed(0)}%` : '—';
        return `<tr onclick='openPmDrawer(${JSON.stringify(m.condition_id)})'>
          ...
          <td>${fmtMoney(m.volume_24h)}</td>
          <td class="${(m.volume_spike_ratio || 0) >= 1.4 ? 'neg' : 'pos'}">${ratio}</td>
        </tr>`;
      }).join('')}</tbody>
    </table>`;
}
  ...
  try {
    ...
    $('drawerBody').innerHTML = `
      ...
      <div class="drawer-section"><div class="pm-detail-stats">
        ...
        <div><span>7D Volume</span><b>${fmtMoney(m.volume_7d)}</b></div>
        <div><span>24h Volume</span><b>${fmtMoney(m.volume_24h)}</b></div>
        <div><span>Avg Vol</span><b>${m.volume_10d_avg ? fmtMoney(m.volume_10d_avg) : '—'}</b></div>
        <div><span>Volume Volatility</span><b class="${(m.volume_spike_ratio || 0) >= 1.4 ? 'neg' : 'pos'}">${m.volume_spike_ratio ? `${(((Number(m.volume_spike_ratio)-1)*100) >= 0 ? '+' : '')}${((Number(m.volume_spike_ratio)-1)*100).toFixed(0)}%` : '—'}</b></div>
      </div></div>
      <div class="drawer-section"><h3>7日概率走势</h3><div id="pmProbChart" class="price-chart"></div></div>
      <div class="drawer-section"><h3>跟踪账户持仓</h3>${renderWhaleTable(m.whale_holders || [])}</div>`;
...
function renderWhaleTable(rows) {
  if (!rows || !rows.length) return `<div class="empty">暂无 tracked wallets 持仓。先点击“同步 Whales”，或该事件暂无被跟踪账户持有。</div>`;
  return `<div class="whale-holder-grid">${rows.slice(0, 4).map(h => {
    const no = String(h.outcome || '').toLowerCase() === 'no';
    return `<article class="whale-holder-card">
      <div class="whale-holder-head">
        <div><h4>${escapeHtml(h.name || '')}</h4><span>${escapeHtml((h.address || '').slice(0, 10))}…</span></div>
        <span class="bucket-pill ${no ? 'neg-badge' : 'pos-badge'}">${escapeHtml(h.outcome || '—')}</span>
      </div>
      <div class="whale-metric-grid">
        <div><span>目前仓位</span><b>${fmtMoney(h.value)}</b></div>
        <div><span>10日最大变化</span><b class="${(h.max_delta_10d || 0) >= 0 ? 'pos' : 'neg'}">${fmtDelta(h.max_delta_10d)}</b></div>
        <div><span>近24h最大变化</span><b class="${(h.max_delta_24h || 0) >= 0 ? 'pos' : 'neg'}">${fmtDelta(h.max_delta_24h)}</b></div>
        <div><span>胜率 / 持有</span><b>${fmtNum(h.win_rate,1)}% · ${escapeHtml(h.days_held_label || ((h.days_held || 0) + 'd'))}</b></div>
      </div>
      <div class="whale-spark-wrap">${renderWhaleSparkline(h.history || [])}</div>
      <div class="spark-legend"><span><i class="yes-dot"></i>YES ${fmtMoney(h.yes_value || 0)}</span><span><i class="no-dot"></i>NO ${fmtMoney(h.no_value || 0)}</span></div>
      <div class="microcopy">${historySourceText(h.history_source)}</div>
    </article>`;
  }).join('')}</div>`;
}

function historySourceText(source) {
  if (source === 'local_position_snapshots') return '曲线来源：本地多次 /positions 快照；当前值与 positions API 对齐';
  if (source === 'trade_backfill_plus_local_snapshots') return '曲线来源：近10日 /trades 回补 + 本地 /positions 快照覆盖；今日与 positions API 对齐';
  if (source === 'trade_history_backfill') return '曲线来源：近10日 /trades 回补 + 今日 /positions 校准';
  if (source === 'held_before_window_no_10d_trades') return '近10日未见该账户交易；当前仓位视为窗口前已持有';
  if (source === 'no_snapshots') return '暂无本地快照';
  return '';
}
