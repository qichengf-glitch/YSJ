...
from .whale_service import get_whale_summary, get_whale_daily_overview, sync_tracked_whales
...
app = FastAPI(title="US Event Intelligence", version="6.3.0")
...
def sync_prediction_market_stack():
    """Automatic full refresh for the Prediction Markets page.

    Runs Polymarket event sync first, then saves a fresh current-position
    snapshot for every tracked wallet. It deliberately uses the same explicit
    /positions snapshot-diff discipline as reference_track_whales.py.
    """
    try:
        sync_prediction_markets(
        ...
        print(f"[prediction-auto-sync] failed: {exc}")
