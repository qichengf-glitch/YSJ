...
GEO_KEYWORDS = [
    "iran", "israel", "ukraine", "russia", "gaza", "hamas", "hezbollah", "houthi",
...
def classify_bucket(event_title: str, question: str, tags: List[Dict[str, Any]]) -> Tuple[Optional[str], str]:
    ...
    # Priority follows trading impact: geopolitical commodities first, then rates/USD, then growth/risk.
    if (slugs & TAG_SLUGS["international"]) or _has_any(text, GEO_KEYWORDS):
        return "geo_commodities", "tags/keywords: geopolitical"
    if (slugs & TAG_SLUGS["treasury"]) or _has_any(text, RATES_KEYWORDS):
        return "rates_usd", "tags/keywords: rates_usd"
    if (slugs & TAG_SLUGS["index"]) or _has_any(text, GROWTH_KEYWORDS):
        return "growth_risk", "tags/keywords: growth_risk"
    return None, "no_macro_bucket"
            ...
            for m in event.get("markets") or []:
                ...
                conn.execute(
                    ...
                    INSERT INTO pm_markets(
                        ...
                        bid, ask, spread, volume_7d, liquidity, signal_type, signal_score,
                    ...
                    (
                        condition_id, event_id, token_id, bucket, bucket_reason, source_category,
                        event_title, question, price_now, price_7d_ago, change_7d, change_1d, change_1mo,
                        bid, ask, spread, volume_7d, liquidity, sig, sig_score,
...
def get_prediction_overview() -> Dict[str, Any]:
    ...
    with get_conn() as conn:
        ...
        row = conn.execute(f"SELECT MAX(fetched_at) AS fetched_at, COUNT(*) AS cnt FROM pm_markets WHERE {macro_clause}", macro_params).fetchone()
    ...
    by_bucket.sort(key=lambda x: {"rates_usd": 0, "geo_commodities": 1, "growth_risk": 2}.get(x["bucket"], 9))

    rising = _top("change_7d > 0", "change_7d DESC, volume_7d DESC", 6)
    falling = _top("change_7d < 0", "change_7d ASC, volume_7d DESC", 6)
    volume_leaders = _top("volume_7d > 0", "volume_7d DESC", 8)
...
def get_prediction_market_detail(condition_id: str) -> Dict[str, Any]:
    with get_conn() as conn:
        ...
        if row is None:
            return {}
        market = format_market_row(dict(row), include_raw=True)
