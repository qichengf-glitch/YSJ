...
def is_recent_enough(value: Any) -> bool:
    ...
    if dt.tzinfo is not None:
        ...
        dt_cmp = dt
    return dt_cmp >= datetime.now() - timedelta(days=7)
...
def process_logs_for(source_type: str) -> Dict[str, Any]:
    ...
    last_log_id = int(last) if last else None
    logs = client.fetch_log(source_type, CATEGORY, last_log_id=last_log_id)
    logs = sorted(logs, key=lambda x: int(x.get("log_id") or 0))
    processed = 0
    max_log_id = last_log_id or 0
    for log in logs:
        action = log.get("action") or "unknown"
        data = log.get("data") or {}
        data_id = int(log.get("data_id") or data.get("id"))
        ...
        if action in {"insert", "update"}:
            ...
            mark_deleted(source_type, data_id, action=action, modify_time=modify_time, fallback_data=data if data else None)
        max_log_id = max(max_log_id, int(log.get("log_id") or 0))
        processed += 1
    if processed:
        set_state(state_key, str(max_log_id))
    set_state(f"last_{source_type}_log_poll_at", utc_now())
    return {"source_type": source_type, "processed": processed, "last_log_id": max_log_id}
